...
export class PaymentService implements IPaymentService {
    ...
    async processPayment(paymentDto: ProcessPaymentDto, traceId?: string): Promise<Payment> {
        ...
        try {
            ...
            this.logger.error("Payment processing error", error as Error, { traceId, transactionId, error: error.message, stack: error.stack });
            throw error;
        }
    }
    ...
    private async handlePaymentFailure(paymentId: string, startTime: number, traceId?: string): Promise<never> {
