...
export class InvoiceService implements IInvoiceService {
    ...
    async generateInvoice(orderId: string, paymentId: string, traceId?: string): Promise<Invoice> {
        try {
            ...
            this.logger.error("Failed to generate invoice", error, { traceId, orderId, paymentId, error: error.message, stack: error.stack });
            throw error;
        }
    }
    ...
    private async fetchOrderDetails(orderId: string, traceId?: string): Promise<any> {
        try {
            ...
            this.logger.error("Failed to fetch order", error, { traceId, orderId, error: error.message, stack: error.stack });
    ...
    private async fetchPaymentDetails(paymentId: string, traceId?: string): Promise<any> {
        try {
            ...
            this.logger.error("Failed to fetch payment", error, { traceId, paymentId, error: error.message, stack: error.stack });
